    ...
    return (
      <div className="mb-4 rounded-lg bg-gradient-to-r from-orange-50 to-red-50 border border-orange-200 px-4 py-2.5 flex items-center justify-between gap-3">
        <div className="text-sm">
          <span className="font-bold text-red-600">⚡ キャンペーン開催中</span>
          <span className="text-gray-600 ml-1">15%OFF終了まで</span>
        </div>
        <div className="flex items-center gap-1 font-mono font-bold text-red-600 text-lg tabular-nums">
          <span className="bg-white px-2 py-0.5 rounded">{pad(timeLeft.hours)}時間</span>
          <span className="bg-white px-2 py-0.5 rounded">{pad(timeLeft.minutes)}分</span>
          <span className="bg-white px-2 py-0.5 rounded">{pad(timeLeft.seconds)}秒</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gradient-to-r from-orange-500 to-red-500 text-white text-center py-3 px-4">
      <div className="max-w-7xl mx-auto flex items-center justify-center gap-4 flex-wrap">
        <span className="font-bold text-sm md:text-base">⚡ キャンペーン実施中 — 全品15%OFF！</span>
        <div className="flex items-center gap-1.5 font-mono font-bold text-lg tabular-nums">
          <span className="bg-white/20 px-2 py-0.5 rounded">{pad(timeLeft.hours)}時間</span>
          <span className="bg-white/20 px-2 py-0.5 rounded">{pad(timeLeft.minutes)}分</span>
          <span className="bg-white/20 px-2 py-0.5 rounded">{pad(timeLeft.seconds)}秒</span>
        </div>
      </div>
    </div>
  );
